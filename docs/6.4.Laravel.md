...
L'única cosa que falta per afegir i configurar correctament perquè tot funcione són les rutes i les vistes, que com hem esmentat en la introducció es generen mitjançant el següent comando: **php artisan make:auth**
En les següents seccions anem a veure les rutes i vistes que es generen i com s'han d'utilitzar.

#### Rutes
Per defecte Laravel no inclou les rutes per al control d'usuaris, per a generar-les hem d'executar el comando php artisan make:auth. Aquest comando ens afegirà les següents línies al fitxer routes/web.php:
...
En executar el comando php artisan make:auth també es generaran totes les vistes necessàries per a realitzar el login, registre i per a recuperar la contrasenya. 
